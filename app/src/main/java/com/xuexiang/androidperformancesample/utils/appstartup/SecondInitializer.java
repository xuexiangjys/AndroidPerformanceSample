 ...
 *
 * 依赖于FirstInitializer的初始化，待其初始化后再初始化
 *
 * @author xuexiang
 * @since 2022/12/6 00:28
 */
public class SecondInitializer implements Initializer<Context> {

    private static final String TAG = "SecondInitializer";

    @NonNull
    @Override
    public Context create(@NonNull Context context) {
        // 4.待SDKInitializer的初始化完毕后，开始初始化
        Log.i(TAG, "Second init start...");
        return context;
    }
    ...
    public List<Class<? extends Initializer<?>>> dependencies() {
        // 1.首先走到这，找寻初始化依赖
        Log.i(TAG, "dependencies:FirstInitializer.class");
        return Collections.singletonList(FirstInitializer.class);
    }
}
